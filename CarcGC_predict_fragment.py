...
####################################Constants Settings###########################
Drug_feature_file = r'CarcGC\drug_graph_feat'
Max_atoms = 100
...
def FragmentGenerate():
    drug_pubchem_id_set = []
    all_drug_feature = {}
    for each in os.listdir(r'CarcGC\drug_graph_feat_fragment'):
        drug_pubchem_id_set.append(each.split('.')[0])
        feat_mat,adj_list,degree_list = hkl.load('%s/%s'%(r'CarcGC\drug_graph_feat_fragment',each))
        all_drug_feature[each.split('.')[0]] = [feat_mat,adj_list,degree_list]
    assert len(drug_pubchem_id_set)==len(all_drug_feature.values())
    label = pd.read_csv(r'Dataset\Carcinogenicity_label_fragment.csv',index_col = None, header = 0)
    drugnames = label['pert_id'].tolist()
    label['Carcinogenicity_label'] = (label['Carcinogenicity'] == '+').astype(int)
    data_idx = list(zip(label['pert_id'],label['Carcinogenicity_label']))
    nb_drugs = len(set([item[0] for item in data_idx]))
...
class MyCallback(Callback):
    ...
    def on_train_end(self, logs={}):
        ...
        self.model.save('CarcGC/bestmodel/BestCarcGC_Cartoxicity_classify_%s.h5'%model_suffix)
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if roc_val > self.best:
            ...
            self.model.save('CarcGC/bestmodel/BestCarcGC_Cartoxicity_highestAUCROC_%s.h5'%model_suffix)
        else:
            self.wait+=1
    ...
    def savedata(self,lr,batchsize,wd,dd):
        ...
        df.to_csv('CarcGC/gridsearch_loss/lr%s_batch%s_dropout%s_%sl2_loss.csv'%(lr,batchsize,dd,wd),index=False,header=True)
        return
...
def ModelTraining(model,lr,batchsize,wd,dd,X_drug_data_train,Y_train,validation_data,nb_epoch=500):
    ...
    callbacks = [ModelCheckpoint(filepath='CarcGC/checkpoint_weight/Cartoxicity_weights_{epoch:04d}.h5',verbose=1),history]
...
def main():
    ...
    auROC_train,auPR_train,Y_pred_train = ModelEvaluate(model,X_drug_data_train,Y_train,r'CarcGC\CarcGC_%s.log'%(model_suffix))
    auROC_validation,auPR_validation,Y_pred_validation = ModelEvaluate(model,X_drug_data_validation,Y_validation,r'CarcGC\CarcGC_test_%s.log'%(model_suffix))
    ...
    X_drug_feat_data_test = np.array(X_drug_feat_data_test)#nb_instance * Max_stom * feat_dim
    X_drug_adj_data_test = np.array(X_drug_adj_data_test)#nb_instance * Max_stom * Max_stom  
    auROC_test,auPR_test,Y_pred_test = ModelEvaluate(model,X_drug_data_test,Y_test,r'CarcGC\CarcGC_test_%s.log'%(model_suffix))

    drug_test_feature,data_test_idx = FragmentGenerate()
    X_drug_data_test,Y_test = FeatureExtract(data_test_idx,drug_test_feature)
    X_drug_feat_data_test = [item[0] for item in X_drug_data_test]
    X_drug_adj_data_test = [item[1] for item in X_drug_data_test]
    X_drug_feat_data_test = np.array(X_drug_feat_data_test)#nb_instance * Max_stom * feat_dim
    X_drug_adj_data_test = np.array(X_drug_adj_data_test)#nb_instance * Max_stom * Max_stom    
    Y_pred = model.predict([X_drug_feat_data_test,X_drug_adj_data_test])
    df1 = pd.DataFrame(Y_pred,index=data_test_idx)
    df1.columns = ['Fragment']
    df1.to_csv('CarcGC/Fragment.csv',index=True,header=True)

    drug_test_feature,data_test_idx = AllDataGenerate()
    X_drug_data_test,Y_test = FeatureExtract(data_test_idx,drug_test_feature)
    X_drug_feat_data_test = [item[0] for item in X_drug_data_test]
    X_drug_adj_data_test = [item[1] for item in X_drug_data_test]
    X_drug_feat_data_test = np.array(X_drug_feat_data_test)#nb_instance * Max_stom * feat_dim
    ...
    Y_pred = model.predict([X_drug_feat_data_test,X_drug_adj_data_test])
    df1 = pd.DataFrame(Y_pred,index=data_test_idx)
    df1.columns = ['Molecule']
    df1.to_csv('CarcGC/Molecule.csv',index=True,header=True)
