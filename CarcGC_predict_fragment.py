...
def FragmentGenerate():
    drug_pubchem_id_set = []
    all_drug_feature = {}
    for each in os.listdir(r'CarcGC_data\drug_graph_feat_fragment'):
        drug_pubchem_id_set.append(each.split('.')[0])
        feat_mat,adj_list,degree_list = hkl.load('%s/%s'%(r'CarcGC_data\drug_graph_feat_fragment',each))
        all_drug_feature[each.split('.')[0]] = [feat_mat,adj_list,degree_list]
    assert len(drug_pubchem_id_set)==len(all_drug_feature.values())
...
def main():
    ...
    df1.to_csv('CarcGC_data/Fragment.csv',index=True,header=True)
    ...
    df1.to_csv('CarcGC_data/Molecule.csv',index=True,header=True)
