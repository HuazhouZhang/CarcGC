...
def dataarrange():
	fdata = pd.read_csv('CarcGC_data/Fragment.csv',index_col=0,header=0)
	mdata = pd.read_csv('CarcGC_data/Molecule.csv',index_col=0,header=0)
	for chemf in fdata.index:
		chemfs = chemf.split('_')
		chem = chemfs[0]
		mc = mdata.loc[chem,'Molecule']
		fdata.loc[chemf,'Molecule'] = mc
	fdata.to_csv('CarcGC_data/Fragment_Molecule.csv',index=True,header=True)

def calweight():
	df = pd.read_csv('CarcGC_data/Fragment_Molecule.csv',index_col=0,header=0)
	...
	for i,chem in enumerate(chemlist):
		...
		fig.savefig('CarcGC_data/Molecule_Weight_figure/%s.png'%chem, bbox_inches='tight')
