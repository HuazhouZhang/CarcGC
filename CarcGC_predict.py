...
def AirDataGenerate():
    drug_pubchem_id_set = []
    all_drug_feature = {}
    for each in os.listdir(r'CarcGC\drug_graph_feat_air'):
        drug_pubchem_id_set.append(each.split('.')[0])
        feat_mat,adj_list,degree_list = hkl.load('%s/%s'%(r'CarcGC\drug_graph_feat_air',each))
        all_drug_feature[each.split('.')[0]] = [feat_mat,adj_list,degree_list]
    assert len(drug_pubchem_id_set)==len(all_drug_feature.values())
...
def main():
    ...
    valres.to_csv('CarcGC/predict_result/lr%s_batch%s_dropout%s_%sl2_validation_res.csv'%(lr,batchsize,dd,wd),index=False,header=False)
    ...
    testres.to_csv('CarcGC/predict_result/lr%s_batch%s_dropout%s_%sl2_extra_res.csv'%(lr,batchsize,dd,wd),index=False,header=False)
    ...
    airres.to_csv('CarcGC/predict_result/haps_result.csv',index=False,header=False)
